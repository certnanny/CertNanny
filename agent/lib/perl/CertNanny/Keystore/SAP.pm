...
sub new
{
    ...
    $self->{PKCS12}->{INDIR} = $certnanny_to_sap_dir;
    $self->{PKCS12}->{OUTDIR} = $sap_to_certnanny_dir;
...
sub installcert {
    ...
    my $sap_to_certnanny_dir = $p12_config->{OUTDIR}; 
    my $old_xml_file = File::Spec->catfile($sap_to_certnanny_dir, $xml_filename);
    my $certnanny_to_sap_dir = $p12_config->{INDIR};
